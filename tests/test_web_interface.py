...
def test_upload_valid_file(mock_geolocate, mock_process, client, sample_log_content):
    ...
    # Create test file
    data = {
        'file': (BytesIO(sample_log_content.encode()), 'auth.log')
    }
    ...
    with client:  # Use client context to access session
        ...
        assert b'Analysis Results - Auth Log Analyzer' in response.data  # Check for page title
...
def test_download_with_report(mock_generate_report, client):
    ...
    assert response.status_code == 200
    assert response.headers['Content-Type'].startswith('text/plain')  # Changed to use startswith
...
def test_attack_origin_map(mock_geolocate, mock_process, client):
    ...
    data = {
        'file': (BytesIO(b'test log content'), 'auth.log')
    }
